...
class SystemIntegrityMonitor {
  ...
  async _flushWindow() {
    ...
    try {
      ...
      logger.error('[GOV] Failed to flush integrity window to DB:', e.message);
  ...
  _enterFailsafe(reason, details) {
    ...
    logger.warn(`[GOV] ⚠️ FAILSAFE MODE ACTIVATED — ${reason}: ${details}`);
    this._logAlert('failsafe_activated', 'CRITICAL', `Failsafe engaged: ${details}`);
  }

  _exitFailsafe() {
    this.failsafeMode = false;
    logger.info('[GOV] ✅ Failsafe mode deactivated — API connectivity restored');
  ...
  async _logAlert(type, severity, message) {
    try {
      ...
      logger.error('[GOV] Failed to log alert to DB:', e.message);
...
class ClinicalPerformanceMonitor {
  ...
  async _loadBaselines() {
    try {
      ...
      if (data) {
        ...
        logger.info('[GOV] Clinical baselines loaded:', Object.keys(this.baselines));
      } else {
        logger.warn('[GOV] No active baselines found — statistical monitoring disabled until baselines are set');
      }
    } catch (e) {
      logger.error('[GOV] Failed to load baselines:', e.message);
    }
  }
  ...
  async _flushPerformanceBuffer() {
    ...
    try {
      ...
      logger.error('[GOV] Failed to flush performance buffer to DB:', e.message);
      if (this.queueEvent) {
        this.queueEvent({
  ...
  async _logClinicalAlert(issue) {
    try {
      ...
      logger.error('[GOV] Failed to log clinical alert to DB:', e.message);
      if (this.queueEvent) {
        this.queueEvent({
  ...
  // ── MODEL DRIFT DETECTION (Phiri: "risk emerges from decisions") ──
  // Runs on a schedule (every 6 hours). Compares recent triage behaviour
  // against a rolling baseline to detect drift in:
  //   1. Nurse disagreement rate (signal that AI output is degrading)
  //   2. Triage level distribution (signal that model behaviour has shifted)
  //   3. Confidence distribution (signal that prompt/model mismatch)
  // Fires governance alerts when thresholds are exceeded.
  ...
  async runDriftDetection() {
    try {
      ...
      const recentRows = recentFeedback.data || [];
      const baselineRows = baselineFeedback.data || [];

      if (recentRows.length >= 5 && baselineRows.length >= 10) {
        const recentDisagreeRate = recentRows.filter(r => r.verdict === 'disagree').length / recentRows.length;
        const baselineDisagreeRate = baselineRows.filter(r => r.verdict === 'disagree').length / baselineRows.length;

        // Alert if disagreement rate increased by >10 percentage points OR doubled
        const absoluteIncrease = recentDisagreeRate - baselineDisagreeRate;
        const relativeIncrease = baselineDisagreeRate > 0 ? recentDisagreeRate / baselineDisagreeRate : 0;

        if (absoluteIncrease > 0.10 || (relativeIncrease > 2.0 && recentDisagreeRate > 0.15)) {
          alerts.push({
            type: 'nurse_disagreement_drift',
            severity: absoluteIncrease > 0.20 ? 'CRITICAL' : 'HIGH',
            detail: `Nurse disagreement rate has increased: ${(baselineDisagreeRate * 100).toFixed(1)}% (30d baseline) → ${(recentDisagreeRate * 100).toFixed(1)}% (7d recent). ${recentRows.length} feedback entries in last 7 days.`,
            action: 'Review recent triage outputs — possible model behaviour change. Check TRIAGE_MODEL version and Anthropic model update announcements.',
            data: {
              baseline_rate: baselineDisagreeRate,
              recent_rate: recentDisagreeRate,
              absolute_increase: absoluteIncrease,
              recent_sample_size: recentRows.length,
      ...
      // ── 2. TRIAGE LEVEL DISTRIBUTION DRIFT ──────────────────────
      // Compare last 7 days to previous 30 days
      const [recentTriages, baselineTriages] = await Promise.all([
        this.supabase.from('triage_logs')
          .select('triage_level')
          .gte('created_at', sevenDaysAgo.toISOString()),
        this.supabase.from('triage_logs')
          .select('triage_level')
      ...
      const recentT = recentTriages.data || [];
      const baselineT = baselineTriages.data || [];

      if (recentT.length >= 20 && baselineT.length >= 50) {
        const levels = ['RED', 'ORANGE', 'YELLOW', 'GREEN'];
        const recentDist = {};
        const baselineDist = {};

        for (const l of levels) {
          recentDist[l] = recentT.filter(t => t.triage_level === l).length / recentT.length;
          baselineDist[l] = baselineT.filter(t => t.triage_level === l).length / baselineT.length;
        }

        // Check each level — alert if any shifted by >15 percentage points
        const driftedLevels = [];
        for (const l of levels) {
          const shift = Math.abs(recentDist[l] - baselineDist[l]);
          if (shift > 0.15) {
            driftedLevels.push({
              level: l,
              baseline: (baselineDist[l] * 100).toFixed(1) + '%',
              recent: (recentDist[l] * 100).toFixed(1) + '%',
              shift: (shift * 100).toFixed(1) + 'pp',
            });
          }
        }

        if (driftedLevels.length > 0) {
          alerts.push({
            type: 'triage_distribution_drift',
            severity: 'HIGH',
            detail: `Triage level distribution has shifted significantly in the last 7 days: ${driftedLevels.map(d => d.level + ' ' + d.baseline + '→' + d.recent).join(', ')}.`,
            action: 'Investigate cause — possible model update, prompt change, or seasonal pattern. Compare with TRIAGE_PROMPT_VERSION.',
            data: {
              baseline_distribution: baselineDist,
              recent_distribution: recentDist,
              drifted_levels: driftedLevels,
              recent_sample_size: recentT.length,
              baseline_sample_size: baselineT.length,
            },
          });
        }
      }

      // ── 3. CONFIDENCE DISTRIBUTION DRIFT ────────────────────────
      const [recentConf, baselineConf] = await Promise.all([
        this.supabase.from('triage_logs')
          .select('confidence')
          .gte('created_at', sevenDaysAgo.toISOString())
      ...
      const recentC = (recentConf.data || []).map(r => r.confidence);
      const baselineC = (baselineConf.data || []).map(r => r.confidence);

      if (recentC.length >= 20 && baselineC.length >= 50) {
        const recentAvg = recentC.reduce((a, b) => a + b, 0) / recentC.length;
        const baselineAvg = baselineC.reduce((a, b) => a + b, 0) / baselineC.length;
        const avgDrop = baselineAvg - recentAvg;

        // Alert if average confidence dropped by >10 points
        if (avgDrop > 10) {
          alerts.push({
            type: 'confidence_drift',
            severity: avgDrop > 20 ? 'CRITICAL' : 'HIGH',
            detail: `Average triage confidence has dropped: ${baselineAvg.toFixed(1)}% (30d baseline) → ${recentAvg.toFixed(1)}% (7d recent). Drop of ${avgDrop.toFixed(1)} points.`,
            action: 'Low confidence may indicate prompt/model mismatch. Review TRIAGE_MODEL and recent AI API changes.',
            data: {
              baseline_avg: baselineAvg,
              recent_avg: recentAvg,
              drop: avgDrop,
              recent_sample_size: recentC.length,
              baseline_sample_size: baselineC.length,
            },
          });
        }
      }

      // ── 4. SUBGROUP DISPARITY — language-level disagree rate ────
      // Yegon: "Disaggregated performance is mandatory"
      // Check if any language has significantly higher nurse disagreement
      if (recentRows.length >= 10) {
        // Get language for each feedback entry from triage_logs
        const recentPatientIds = [...new Set(recentRows.map(r => r.patient_id).filter(Boolean))];
        if (recentPatientIds.length > 0) {
          const { data: langTriages } = await this.supabase
            .from('triage_logs')
            .select('patient_id, language')
            .in('patient_id', recentPatientIds.slice(0, 200))
            .not('language', 'is', null);

          if (langTriages && langTriages.length > 0) {
            const patientLang = {};
            for (const t of langTriages) patientLang[t.patient_id] = t.language;

            const langFeedback = {};
            for (const r of recentRows) {
              const lang = patientLang[r.patient_id] || 'unknown';
              if (!langFeedback[lang]) langFeedback[lang] = { total: 0, disagree: 0 };
              langFeedback[lang].total++;
              if (r.verdict === 'disagree') langFeedback[lang].disagree++;
            }

            const overallRate = recentRows.filter(r => r.verdict === 'disagree').length / recentRows.length;
            for (const [lang, data] of Object.entries(langFeedback)) {
              if (lang === 'unknown' || data.total < 3) continue;
              const langRate = data.disagree / data.total;
              if (langRate > overallRate + 0.15 && langRate > 0.20) {
                alerts.push({
                  type: 'language_equity_drift',
                  severity: 'HIGH',
                  detail: `${lang} patients have ${(langRate * 100).toFixed(1)}% nurse disagreement vs ${(overallRate * 100).toFixed(1)}% overall (${data.total} entries). Possible equity issue — AI may perform worse for this language.`,
                  action: `Review DCSL keyword coverage and AI prompt handling for ${lang}. Check native speaker DCSL validation.`,
                  data: { language: lang, lang_rate: langRate, overall_rate: overallRate, sample_size: data.total },
                });
              }
            }
          }
      ...
      for (const alert of alerts) {
        await this._logClinicalAlert(alert);
      }

      if (alerts.length > 0) {
        logger.warn(`[DRIFT] ${alerts.length} drift alert(s) fired: ${alerts.map(a => a.type).join(', ')}`);
      } else {
        logger.info('[DRIFT] No drift detected');
      }

      return { status: 'completed', alerts_fired: alerts.length, checked_at: now };
    } catch (e) {
      logger.error('[DRIFT] Detection failed:', e.message);
...
class StrategicLifecycleMonitor {
  ...
  async checkReviewSchedule() {
    try {
      ...
      logger.error('[GOV] Review schedule check failed:', e.message);
      return { status: 'error', error: e.message };
    }
  }
  ...
  async checkEnrollmentThresholds() {
    try {
      ...
      logger.error('[GOV] Enrollment check failed:', e.message);
      return { status: 'error', error: e.message };
    }
  }
  ...
  async checkClinicianFeedback() {
    try {
      ...
      logger.error('[GOV] Feedback check failed:', e.message);
      return { status: 'error', error: e.message };
    }
  }
  ...
  async checkFalseNegativeRate() {
    try {
      ...
      logger.error('[GOV] False negative check failed:', e.message);
      return { status: 'error', error: e.message };
    }
  }
  ...
  async _triggerReview(type, daysSinceDeployment, reviewNumber = 1) {
    try {
      ...
      logger.error('[GOV] Failed to trigger review:', e.message);
    }
  }
  ...
  async _logStrategicAlert(issue) {
    try {
      ...
      logger.error('[GOV] Failed to log strategic alert:', e.message);
    }
  }
}
...
class IncidentManager {
  ...
  async reportIncident(report) {
    ...
    try {
      ...
      logger.error('[GOV] Failed to report incident:', e.message);
      return { success: false, error: e.message };
    }
  }

  async _triggerStopWork(incident) {
    logger.error(`[GOV] 🛑 STOP-WORK TRIGGERED — Level ${incident.severity_level}: ${incident.description}`);
  ...
  async resolveIncident(incidentId, resolution) {
    ...
    try {
      ...
      logger.error('[GOV] Failed to resolve incident:', e.message);
      return { success: false, error: e.message };
    }
  }
  ...
  async runMonthlyAudit() {
    try {
      ...
      logger.error('[GOV] Monthly audit failed:', e.message);
      return { status: 'error', error: e.message };
    }
  }
  ...
  async _logIncidentAlert(incident) {
    try {
      ...
      logger.error('[GOV] Failed to log incident alert:', e.message);
    }
  }
}
...
class GovernanceOrchestrator {
  ...
  _scheduleAgents() {
    ...
    setInterval(() => this.clinicalPerformance.runDriftDetection(), 6 * 60 * 60 * 1000);
    // Also run drift detection 10 minutes after startup (let data accumulate)
    setTimeout(() => this.clinicalPerformance.runDriftDetection(), 10 * 60 * 1000);
